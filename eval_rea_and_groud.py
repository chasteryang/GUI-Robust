...
def main(args):
    ...
    # print(is_abnormal)
    model = None
    model_reason = Qwen2_5_VL()
    model_ground = UI_TARS()
    model_reason.__init__()
    model_ground.__init__()
    ...
    if(eval_type == "step"):
        ...
        for task in tqdm(task_to_run):
            ...
            """
            try:
                pred_0 = model_reason.pred_task_full(task_description, base64_image_list) #TODO: model.pred_task_full
                pred = model_ground.pred_task_ground(pred_0, base64_image_list)
                # print("hi")
            except Exception as e:
                print(f"[{task_description}] 模型调用失败: {e}")
                pred = None
        ...
        # print(results)
        # with open(output_file, 'a', encoding='utf-8') as f:
        output_file = "./result/result_task_" + model_name + "_rebuttal.json"
        output_data = [
            {
